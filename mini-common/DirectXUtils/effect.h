...
namespace mini
{
	...
	class ComputeShaderComponent : public EffectComponent
	{
	public:
		explicit ComputeShaderComponent(dx_ptr<ID3D11ComputeShader>&& cs = nullptr)
			: m_cs(move(cs))
		{}

		ComputeShaderComponent(ComputeShaderComponent&& other) noexcept
			: m_cs(move(other.m_cs))
		{}

		ComputeShaderComponent& operator=(ComputeShaderComponent&& other) noexcept
		{
			SetComputeShader(move(other.m_cs));
			return *this;
		}
		...
		void SetComputeShader(dx_ptr<ID3D11ComputeShader>&& cs) { m_cs = move(cs); }

		dx_ptr<ID3D11ComputeShader> m_cs;
	};
...
#define DEFINE_RESOURCE_SET_CLASS(SHADER_TYPE, RESOURCE_TYPE) \
	...
	using UnorderedAccessViewSet = ResourceSet<ID3D11UnorderedAccessView>;
	class CSUnorderedAccessViews : public EffectComponent, public UnorderedAccessViewSet
	{
	public:
		template<typename T>\
		using dx_ptr = mini::directx::dx_ptr<T>;\
		using base_set_type = UnorderedAccessViewSet;\
		using pointer = base_set_type::pointer;\
		using value_type = base_set_type::value_type;\
		using unique_ptr_type = base_set_type::unique_ptr_type; \
		using base_set_type ::ResourceSet;\
		void SET_RESOURCE_F_NAME(CS, UnorderedAccessView) (unsigned slot, pointer buffer) {\
			SetResource(slot, buffer);\
		}\
		void SET_RESOURCE_F_NAME(CS, UnorderedAccessView) (unsigned slot, unique_ptr_type&& buffer) {\
			SetResource(slot, std::move(buffer));\
		}\
		void SET_RESOURCE_F_NAME(CS, UnorderedAccessView) (unsigned slot, const unique_ptr_type& buffer) {\
			SetResource(slot, buffer);\
		}\
		void Begin(const dx_ptr<ID3D11DeviceContext>& context) const override {\
			context-> CONTEXT_SET_F_NAME(CS, UnorderedAccessView) (0, static_cast<unsigned>(m_buffers.size()), m_buffers.data(), nullptr);\
		}\
		dx_ptr_vector<value_type>& GET_RESOURCES_VECTOR_F_NAME(CS, UnorderedAccessView) ()\
		{ return m_buffers; }\
		const dx_ptr_vector<value_type>& GET_RESOURCES_VECTOR_F_NAME(CS, UnorderedAccessView) () const\
		{ return m_buffers; }\
	};
...
#define DEFINE_SHADER_RESOURCE_SET_CLASSES(RESOURCE_TYPE, PTR_TYPE) \
	...
	DEFINE_RESOURCE_SET_CLASS(CS, RESOURCE_TYPE);
	...
	public:
		...
		RenderTargetsEffect(const directx::viewport &viewport, size_t target_count = 0U)
			: ResourceSet{ target_count }, m_viewport{ viewport}, m_depthBuffer{ nullptr }, m_clearOnBegin{ false }
		{ }

		template<typename... ARGS>
		explicit RenderTargetsEffect(directx::viewport viewport, dx_ptr<ARGS>&& ...args)
