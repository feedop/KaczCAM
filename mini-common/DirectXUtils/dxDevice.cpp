...
{
	assert(m_device);
	ID3D11ComputeShader* cp = nullptr;
	auto hr = m_device->CreateComputeShader(byteCode, byteCodeSize, nullptr, &cp);
	dx_ptr<ID3D11ComputeShader> computeShader(cp);
	if (FAILED(hr))
		throw utils::winapi_error{ hr };
...
dx_ptr<ID3D11UnorderedAccessView> DxDevice::CreateUnorderedAccessView(ID3D11Resource* texture,
																	const unordered_access_view_info* desc) const
{
	assert(m_device);
	ID3D11UnorderedAccessView* uav;
	auto hr = m_device->CreateUnorderedAccessView(texture, desc, &uav);
	dx_ptr<ID3D11UnorderedAccessView> resourceView(uav);
	if (FAILED(hr))
		throw utils::winapi_error{ hr };
	return resourceView;
}
