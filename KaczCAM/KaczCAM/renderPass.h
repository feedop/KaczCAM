...
namespace mini
{
	...
	namespace gk2
	{
		class RenderPass
		{
			...
			public:
				...
				static_assert(std::is_base_of<EffectComponent, ConstantBufferEffectT>::value &&
					...
					"Base class of CBVariableEffect needs to be a shader constant buffer effect type");
		...
		public:
			...
			void Execute(const dx_ptr<ID3D11DeviceContext>& context, CBVariableManager& manager, bool compute = false);
		...
		private:
			...
			std::vector<CBufferDesc> _getConstantbuffer_infos(const dx_ptr<ID3D11ShaderReflection>& shaderRefl, const D3D11_SHADER_DESC& shaderDesc);
			...
			{
				std::vector<CBufferDesc> buffers = _getConstantbuffer_infos(shaderRefl, shaderDesc);
				if (!buffers.empty())
				{
			...
			template<typename ShaderResourcesEffectT>
			void _addShaderUAVs(const CBVariableManager& variables, const dx_ptr<ID3D11ShaderReflection>& shaderRefl, const D3D11_SHADER_DESC& shaderDesc)
			{
				std::vector<std::string> uavNames = _getNames(shaderRefl, shaderDesc, D3D_SIT_UAV_RWTYPED);
				if (!uavNames.empty())
				{
					auto uptr = std::make_unique<ShaderResourcesEffectT>();
					uptr->m_buffers.reserve(uavNames.size());
					for (size_t i = 0; i < uavNames.size(); ++i)
						if (!uavNames[i].empty())
							uptr->SetResource(static_cast<UINT>(i), variables.GetUAV(uavNames[i]));
					m_effect.m_components.push_back(std::move(uptr));
				}
			}
