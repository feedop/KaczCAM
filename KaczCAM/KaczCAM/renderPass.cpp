...
void RenderPass::_initShaders(const DxDevice& device, const CBVariableManager& variables,
							  const wstring& csShader)
{
	const auto csCode = DxDevice::LoadByteCode(csShader);
	auto cs = device.CreateComputeShader(csCode);
	AddEffect(make_unique<ComputeShaderComponent>(move(cs)));
	D3D11_SHADER_DESC desc;
	auto csRefl = _reflectShader(csCode, desc);
	_addShaderConstantBuffers<CSConstantBuffers>(device, csRefl, desc);
	_addShaderSamplers<CSSamplers>(variables, csRefl, desc);
	_addShaderTextures<CSShaderResources>(variables, csRefl, desc);
	_addShaderUAVs<CSUnorderedAccessViews>(variables, csRefl, desc);
}
...
RenderPass::RenderPass(const DxDevice& device, const CBVariableManager& variables, InputLayoutManager* layouts,
	...
	: m_layouts(layouts)
{
	_initShaders(device, variables, csShader);
}
...
void RenderPass::Execute(const dx_ptr<ID3D11DeviceContext>& context, CBVariableManager& manager, bool compute)
{
...
vector<CBufferDesc> RenderPass::_getConstantbuffer_infos(const dx_ptr<ID3D11ShaderReflection>& shaderRefl, const D3D11_SHADER_DESC& shaderDesc)
{
