...
Model ModelLoader::LoadFromCustomFile(const string& filename, InputLayoutManager& layouts, bool smoothNormals)
{
	ifstream input;
	...
	Importer importer;
	initLoader(importer);
	return convertToModel(importer.ReadFileFromMemory(data.data(), data.size(), getImportFlags(smoothNormals)), layouts);
}
